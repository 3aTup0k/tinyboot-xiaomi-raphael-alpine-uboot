...
echo "[$(date +'%Y-%m-%d %H:%M:%S')] [15] 🧹 Cleaning up temporary files"

export DEBIAN_FRONTEND=noninteractive

echo "[$(date +'%Y-%m-%d %H:%M:%S')] [15]   └─ Cleaning apt-get cache"
...
echo "[$(date +'%Y-%m-%d %H:%M:%S')] [15]   └─ Cleaning firmware files"
rm -f rootdir/lib/firmware/reg* 2>/dev/null || true

echo "[$(date +'%Y-%m-%d %H:%M:%S')] [15]   └─ Configuring Tsinghua mirrors"
...
EOF
    ...
    cat > rootdir/etc/apt/sources.list << EOF
deb $DEBIAN_MIRROR $DEBIAN_VERSION main non-free contrib
...
cat rootdir/etc/default/zramswap 2>/dev/null || echo "(File does not exist)"

echo ""
echo "========================================== 📋 Configuration Preview End =========================================="

echo "[$(date +'%Y-%m-%d %H:%M:%S')] [15] ✅ Cleanup completed"
